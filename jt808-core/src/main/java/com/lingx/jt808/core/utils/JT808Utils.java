...
import com.lingx.jt808.core.bean.RetBean;
...
public class JT808Utils {
	...
	public static String getResKey(String tid,int msgId,int msgSn) {
		return String.format("%s_%s", Utils.parseTid(tid),msgSn);
	}
	/**
	 * TID + msgId
	 * @param tid
	...
	public static String getResKey2(String tid,int msgId,int msgSn) {
		return String.format("%s_%s", Utils.parseTid(tid),msgId);
	}
	...
	public static String getTidByVersion(String tid,boolean isVersion) {
		if(isVersion) {
